...
3. Install the Godot engine (version 4.4.1) from https://godotengine.org/ and run it.
    > [!NOTE]
    > You can use the [`godotenv`](https://github.com/chickensoft-games/GodotEnv) tool to help with this.
    > Just run ``dotnet tool run godotenv godot install 4.3.0`` and ``dotnet tool run godotenv godot env setup`` then restart your shell and everything should be ready to go.
    > Either way, you'll need a ``GODOT`` environment variable pointing to the godot executable.
